...
## 1.0 当前 v0.0.7.x closure audit mode

当前项目处于 v0.0.7.x closure audit mode。P0-A / P0-B / P0-C 已解决，但 v0.0.7.x 尚未安全 closure。
...
Codex 当前任务重点是只读审计、债务分流、docs alignment；不得自行推进机制内容。
...
```text
v0.0.7.x closure audit mode
```

P0-A / P0-B / P0-C 已解决，但这不代表 v0.0.7.x 已安全 closure，也不代表 P1 solved。

当前阶段重点是只读审计、债务分流和必要的 docs alignment；不得自行推进机制内容、batch content、generated severity、shadow / partial / active takeover 或 v0.0.8.x 开工。

历史阶段说明：

- v0.0.5.x：现有核心系统 ID 化 / 去显示文案主键 / 平台无关数据地基。
- v0.0.6.x：三层属性 / profile / summary 地基。
- v0.0.7.x：原计划进入 severity / threshold / scoreMultiplier / 内容管线调参；当前已从 P0 docs recovery 转入 closure audit mode。

closure audit 完成并经 ChatGPT + 用户确认前，不得把 v0.0.7.x 当作安全 closure，也不得开 v0.0.8.x。

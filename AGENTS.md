...
## 1.0 当前 v0.0.7 docs closure checkpoint / v0.0.8 planning-only

当前项目已到达 v0.0.7 docs closure checkpoint。`docs/V0_0_8_PLANNING_TODO.md` 创建并登记后，可作为当前 active stage planning TODO；implementation 仍关闭，除非用户明确批准。
...
- 不让 Codex 发明机制概念。
- 不把 planning TODO 视为 implementation permission。
...
6. `docs/V0_0_8_PLANNING_TODO.md`
   - 当前 v0.0.8.x planning TODO / active stage TODO；planning-only，不开放 implementation。
...
```text
v0.0.7 docs closure checkpoint reached；v0.0.8 planning TODO may be active once registered；implementation still closed unless explicitly approved
```

P0-A / P0-B / P0-C 已解决，但这不代表 P1 solved，也不代表 v0.0.8.x implementation 已开放。

当前阶段重点是 planning TODO、债务分流和必要的 docs alignment；不得自行推进机制内容、batch content、generated severity、shadow / partial / active takeover 或 v0.0.8.x implementation。
...
- v0.0.8.x：当前仅允许 planning TODO / 内容管线施工图；implementation 需要用户另行明确批准。

v0.0.8 planning TODO 不等于 implementation permission；不得从 planning TODO 直接进入 runtime / generated / data / golden 修改。

...
| `flavor_identity_conflict` | outcomeTypeId | `core/tasteJudge.js`, `data/goldenSamples.js`, generated feedback, `content_sheets/examples/feedback_texts.sample.*`, `content_sheets/examples/candidate_severity_rules.sample.*`, docs | runtime mapping / golden / generated / sample / docs | current migrated outcomeTypeId | Needs note | keep as current outcomeTypeId; not feedbackTag / candidateId / sampleId / severityLevel; source should be explained through `sourceLayer=flavor`, `sourceSummary=flavorSummary`, `triggerMetric=identityConflictRisk`, with related candidate / risk tag `identity_conflict` | before outcome / severity validator |
| `taste_conflict` | legacy outcomeTypeId | docs / reports historical notes | pre-v0.0.7.41 history | legacy / pre-migration / historical | Historical note | do not use as current effective field; future validator / registry / generated data / runtime should not treat it as current value; keep only when labeled legacy / pre-migration / historical | none |
...
| `bubble_conflict` | feedbackTag | `data/feedbackTexts.js`, generated feedback, `data/goldenSamples.js`, docs | runtime / generated / golden / docs | runtime observed feedbackTag with narrow semantics | Needs note | keep narrow semantics: 气泡 + 厚重 / 口感冲突追评；do not reuse for generic flavor identity conflict without feedbackTag mapping review / producer review | before severity feedbackTag references |
| `aroma_pressure`, `identity_conflict`, `low_beverage_fit`, `savory_identity`, `texture_sediment`, `novelty` | candidateTag / risk tag, not runtime feedbackTag | `core/summaryCandidateEngine.js`, docs | readonly summary candidates / docs | candidate-only / needs source-of-truth | Needs review | do not register yet; keep as candidate / risk semantics until feedbackTag registry confirms; `identity_conflict` must not become outcomeTypeId just because it resembles `flavor_identity_conflict` | blocks severity / feedback validator consumption |
...
### v0.0.7.41 迁移后的 outcome / tag 边界

- `flavor_identity_conflict` 是当前 `outcomeTypeId`，由 v0.0.7.41 从 legacy `taste_conflict` 迁移而来，表达“风味身份不协调 / flavor identity conflict”。
- 推荐解释链路是 `sourceLayer=flavor`、`sourceSummary=flavorSummary`、`triggerMetric=identityConflictRisk`，相关 candidate / risk tag 是 `identity_conflict`。
- `flavor_identity_conflict` 不是 feedbackTag、candidateId、sampleId 或 severityLevel，也不表示所有风味问题；它只表示 outcome 层的风味身份冲突。
- `identity_conflict` 仍是 candidate / risk tag，不是 runtime feedbackTag，不能直接进入玩家文案选择，也不能因为字符串接近 `flavor_identity_conflict` 就被当成 outcomeTypeId。
- `bubble_conflict` 仍是窄语义 runtime observed feedbackTag，语义偏气泡 + 厚重 / 口感冲突追评；它不是 `flavor_identity_conflict` 的默认或唯一文案标签。
- legacy `taste_conflict` 只应作为 pre-v0.0.7.41 / historical outcomeTypeId 出现在历史 docs / reports 中；不要删除历史，但也不要把它误读成当前事实。
...
| `flavor_identity_conflict` | 当前 outcomeTypeId；由 v0.0.7.41 从 legacy `taste_conflict` 迁移而来，不是 feedbackTag / candidateId / sampleId / severityLevel | Needs note | keep; source 由 `sourceLayer` / `sourceSummary` / `triggerMetric` 解释 |
| `taste_conflict` | legacy / pre-v0.0.7.41 outcomeTypeId，仅作历史记录 | Historical note | 不再作为当前有效字段使用 |
| `bubble_conflict` | runtime observed feedbackTag，语义偏气泡 + 厚重 / 口感冲突追评 | Needs note | 不泛化到 flavor identity conflict |
| `aroma_pressure` | summary candidate / risk tag，不是 runtime feedbackTag | Needs review | 不注册；如需 feedbackTag 需另行 registry review |
| `identity_conflict` | flavor identity candidate / risk tag，不是 runtime feedbackTag，也不是 outcomeTypeId | Needs review | 不注册；需 registry / shadow data 确认 |

...
| `flavor_identity_conflict` | outcomeTypeId | `core/tasteJudge.js`, `data/goldenSamples.js`, generated feedback, `content_sheets/examples/feedback_texts.sample.*`, `content_sheets/examples/candidate_severity_rules.sample.*`, docs | runtime mapping / golden / generated / sample / docs | current migrated outcome | Needs note | keep as current outcomeTypeId; do not infer source only from name; source should be explained through `sourceLayer=flavor`, `sourceSummary=flavorSummary`, `triggerMetric=identityConflictRisk` | before outcome / severity validator |
| `taste_conflict` | legacy outcomeTypeId | docs / reports historical notes | pre-v0.0.7.41 history | legacy / pre-migration | Historical note | do not use as current effective field; keep only when labeled legacy / pre-migration / historical | none |
...
| `flavor_identity_conflict` | 当前 outcomeTypeId；由 v0.0.7.41 从 legacy `taste_conflict` 迁移而来，不是 feedbackTag | Needs note | keep; source 由 `sourceLayer` / `sourceSummary` / `triggerMetric` 解释 |

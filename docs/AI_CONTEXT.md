...
【不要删】详细的三层 profile、stable ingredientId、三层 summary、事故优先级重排、质地事故细分、粗吸管需求、legacy 迁移原则和 v0.0.5.x 房梁阶段规划，已写入 `docs/TASTE_ENGINE_ARCHITECTURE.md`。新对话或 Codex 在继续 v0.0.5.10 前，应先读取该文档；不要继续机械迁移单个旧事故规则。

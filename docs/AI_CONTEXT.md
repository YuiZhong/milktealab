...
当前下一步路线：不要继续分批新建 report，也不要直接推进 active takeover / generated / golden。v0.0.8.21 已把 legacy final score / accident / type route 隔离成 debug 对照；v0.0.8.22 已补最小 non-final scoring registry / validator gate；v0.0.8.24 开始 score-only playtest takeover scaffold。后续调优应继续走 pressure / balance / aggregation 的通用规则，不得写 recipe whitelist、sample-specific if 或 displayName / 中文名特判。

...
- 最新 main：本轮 docs commit 是 `v0.0.5.26-candidate` 之后的文档同步 commit，提交后以 `git log -1` 为准；本轮不创建 tag，不推进 `v0.0.5.27`。
...
- 当前未推进 v0.0.5.27。下一步可考虑 golden runner 支持 `drinkTypeId` 断言；后续还可评估 `audienceId`、规则表 refs、反馈 tag 边界和 ID 化收口审计。不要为了“干净”批量迁移全部规则表，也不要为未来尚不存在系统提前造空架子。
...
【可删】v0.0.5.26-candidate 已冻结并推送，指向 `eba057291777dbb2d415ff6a511b60209a2065cb`。v0.0.5.26 已完成普通饮品类型 drinkTypeId 双轨地基；普通饮品类型 result 暴露 `drinkTypeId`，`inferType` 保持旧返回值兼容，`inferTypeResult` 提供 `{ type, drinkTypeId }` 结构化返回能力；事故路径继续使用 `accidentTypeId`，没有被 `drinkTypeId` 洗白。正式 tag `v0.0.5.26` 未创建；当前未推进 v0.0.5.27；工作区应为干净状态，golden samples 应为 20/20 passed。

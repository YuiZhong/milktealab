...
当前下一步路线：不要继续分批新建 report，也不要直接推进 active takeover / generated / golden。v0.0.8.21 已把 legacy final score / accident / type route 隔离成 debug 对照；v0.0.8.22 已补最小 non-final scoring registry / validator gate。后续如继续 implementation，可在该门禁上推进 unified pressure-balance scoring scaffold 或 high sweetness overload 的 non-final scoring suggestion rule。任何实现都不得写 recipe whitelist、sample-specific if 或 displayName / 中文名特判。

...
当前 verified baseline：latest main 为 `0302f7d chore: fix profile draft shadow runner parsing`，该 commit 已推 main；新对话继续前应先确认 branch 为 `main`、working tree clean、HEAD 与 `origin/main` 一致。
...
当前下一步路线：不要继续分批新建 report，也不要直接推进 runtime / generated / golden。v0.0.8.19 已把 calibration blockers 收束进 active TODO；下一刀 implementation 建议从 high sweetness overload 的 non-final scoring suggestion rule 开始，随后处理 bitterPressure / dairySupport / sweetnessBalance 的通用 pressure balance shape。任何实现都不得写 recipe whitelist、sample-specific if 或 displayName / 中文名特判。

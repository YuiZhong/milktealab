...
- v0.0.6.1 main 已追加 `tasteSummaryEngine` 拆分 bugfix commit：`core/tasteSummaryEngine.js` 承载 summary 构建逻辑，`core/tasteJudge.js` 回到调度层，只调用 summary 构建入口。
- golden samples 当前应为 `20/20 passed`；当前未创建 v0.0.6.1 tag。
- `v0.0.6.1-candidate` 仍未创建；下一步仍是 v0.0.6.1-candidate 冻结前复查。不要为了“干净”批量迁移全部规则表，也不要为未来尚不存在系统提前造空架子。

...
## 0. 当前 v0.0.7 docs closure checkpoint / v0.0.8 planning-only

当前项目已到达 v0.0.7.x docs-only closure checkpoint，并进入 v0.0.8 planning TODO 建立 / planning-only 状态。
...
- 边界：planning-only，不开放 implementation。

当前限制：

- 不开 v0.0.8.x implementation。
- 不做 batch content。
...
下一步应由用户 + ChatGPT 决策：是否继续完善 planning TODO、是否讨论 tag、或是否准备下一轮明确批准的 implementation task。下一步不是自动 implementation。
...
4. `docs/V0_0_8_PLANNING_TODO.md`
   - 当前 active stage planning TODO。
   - 只规划内容管线、review pack、draft ID、registry candidate 和 validator gate，不开放 implementation。
...
## 6. v0.0.8 planning-only 当前路线

v0.0.7.x 已从 P0 docs recovery 转入 docs-only closure checkpoint reached，并通过 `v0.0.7-docs-closure-checkpoint` 记录 checkpoint。

当前新阶段只建立 `docs/V0_0_8_PLANNING_TODO.md` 作为 planning TODO / active stage TODO。

这不是 v0.0.8.x implementation 开工，不是 v0.0.7.x formal safe closure / candidate / formal tag，也不代表 P1 solved。

batch accidentTypeId / feedbackTag / triggerMetric / ingredient profile / threshold / severity / score / review pack / generated severity / generated severity shadow / partial / active takeover / active validator / runtime takeover，仍全部暂停，除非后续用户明确批准进入 implementation task。Existing generated feedback shadow 如存在，仍是 non-final，不影响 final feedback / final result。

...
当前下一步路线：不要继续分批新建 report，也不要直接推进 active takeover / generated / golden。v0.0.8.21 先把 legacy final score / accident / type route 隔离成 debug 对照；后续如继续 implementation，应在该边界上再推进 high sweetness overload 的 non-final scoring suggestion rule 或 registry / validator 前置债。任何实现都不得写 recipe whitelist、sample-specific if 或 displayName / 中文名特判。

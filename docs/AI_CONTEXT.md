...
## 0. 当前 v0.0.8 playtest default unified judgment / calibration stage

当前项目已从 v0.0.7.x docs closure checkpoint 进入 v0.0.8.x playtest default unified judgment / anchor profile / debug output / human-readable review pack / early calibration stage。
...
- 不开放 active generated severity / final production takeover；默认不改 final score / feedback / result.type / golden expected。显式 debug flag 下可做 score-only takeover 或 playtest unified judgment takeover，但必须可回滚，不写 golden / data/generated。
...
当前下一步路线：不要继续分批新建 report，也不要直接推进 generated / golden / final production takeover。v0.0.8.21 已把 legacy final score / accident / type route 隔离成 debug 对照；v0.0.8.22 已补最小 non-final scoring registry / validator gate；v0.0.8.24 开始 score-only playtest takeover scaffold；v0.0.8.25 开始 playtest unified judgment takeover scaffold；v0.0.8.26 已补 stable ingredient composition tags；v0.0.8.27 开始 playtest composable drink type labeler；v0.0.8.28 开始 playtest unified feedback composer；v0.0.8.30 默认页面已切到 unified judgment + v0.0.8.15 anchor profile；v0.0.8.31 已做第一轮 unified scoring calibration。后续应继续基于试玩反馈小步校准 scoring / balance / support，不得写 recipe whitelist、sample-specific if、legacy feedback if 或 displayName / 中文名特判。

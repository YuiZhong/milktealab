...
当前 verified baseline：latest main 为 `71f3fae chore: clean display text key debt`，该 commit 已推 main；新对话继续前应先确认 branch 为 `main`、working tree clean、HEAD 与 `origin/main` 一致。
...
当前下一步路线：不要继续分批新建 report，也不要直接推进 generated / golden / final production takeover。v0.0.8.21 已把 legacy final score / accident / type route 隔离成 debug 对照；v0.0.8.22 已补最小 non-final scoring registry / validator gate；v0.0.8.24 开始 score-only playtest takeover scaffold；v0.0.8.25 开始 playtest unified judgment takeover scaffold。后续调优应继续走 pressure / balance / aggregation / judgment 的通用规则，不得写 recipe whitelist、sample-specific if 或 displayName / 中文名特判。

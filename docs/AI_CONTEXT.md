...
当前 verified baseline：latest main 为 `a72601b chore: add profile value scale recalibration draft`，该 commit 已推 main；新对话继续前应先确认 branch 为 `main`、working tree clean、HEAD 与 `origin/main` 一致。
...
当前下一步路线：不要继续分批新建 report，也不要直接推进 runtime / generated / golden。v0.0.8.15 已生成 anchor scale correction 草案工作台；下一步应先做制作人 / ChatGPT review、source-limited 项补证、schema gap 讨论，或在明确 gate 后推进下一刀草案校准。

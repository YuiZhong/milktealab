...
当前路线提醒：`c0658dc` 已完成 first read-only generated severity shadow proof；`993a6c1` 已完成 multi-sample generated severity shadow output，并将 `matched` 改为 observation-style `metric_observed_positive`；`949ab2a` 已完成 human-readable generated severity shadow review pack。后续默认产品优先 / 可见产物优先，不再为每个中间判断新增 planning report / review record；`reports/human_review/` 只放真正需要人类 / 制作人审核的材料。

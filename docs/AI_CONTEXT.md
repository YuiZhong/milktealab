...
- 最新 main：本轮 docs commit 是 `v0.0.6.14-candidate` 之后的状态同步 commit，提交后以 `git log -1` 为准。
...
- 当前未推进 v0.0.6.15。
- 下一步可考虑 `v0.0.6.15｜candidate priority shell 只读地基`，或 `v0.0.6.15｜priority shell golden 结构断言设计`，或 v0.0.6.x 后半段收口复盘。不要把下一步写成已经决定，不要为了“干净”批量迁移全部规则表，也不要为未来尚不存在系统提前造空架子。

...
- golden samples 当前应为 `20/20 passed`。
- 当前未推进 v0.0.6.5；下一步可考虑 `v0.0.6.5｜flavorSummary` docs / schema 复核、`v0.0.6.5｜flavorSummary` 只读地基，或先做 v0.0.6.x 中段轻量复盘。不要把下一步写成已经决定，不要为了“干净”批量迁移全部规则表，也不要为未来尚不存在系统提前造空架子。

...
- 最新 main：本轮 v0.0.6.14 本地 docs / schema commit 是 `v0.0.6.13-candidate` 之后的 commit，提交后以 `git log -1` 为准。
...
- 当前未创建 `v0.0.6.14-candidate`。
- 下一步可考虑 `v0.0.6.14-candidate` 冻结，或 `v0.0.6.15｜candidate priority shell 只读地基`，或 `v0.0.6.15｜priority shell golden 结构断言设计`。不要把下一步写成已经决定，不要为了“干净”批量迁移全部规则表，也不要为未来尚不存在系统提前造空架子。

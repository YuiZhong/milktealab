...
- 下一步可考虑 `v0.0.7.13｜feedback runtime adapter 结构保护`、`v0.0.7.13｜feedbackEngine 旁路读取 generated data docs / schema`，或继续 feedback 文案评审会 / 样例扩充。不要把下一步写成已经决定，不要为了“干净”批量迁移全部规则表，也不要为未来尚不存在系统提前造空架子。

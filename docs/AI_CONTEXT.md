...
当前下一步路线：不要继续分批新建 report，也不要直接推进 runtime / generated / golden。v0.0.8.12 已生成 source-referenced first pass 草案工作台；下一步应先做制作人 / ChatGPT review、source-limited 项补证、profile value calibration 讨论，或在明确 gate 后推进下一刀草案校准。

v0.0.8.12 已覆盖 37/37 原料。资料不足的原料已标明 low confidence / source_limited / needs follow-up；这些标记不是失败，而是后续人工复核入口。

v0.0.8.12 仍未新增 report，未新建 planning 文档，未改 runtime profile，未写 `data/generated`，未改 scoring logic，未改 final result，也未为了目标分数或校正测试反推 ingredient profile。联网资料是参考基线，不是游戏真理；最终仍需制作人 review。

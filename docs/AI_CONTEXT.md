...
当前 verified baseline：latest main 为 `f52289c feat: add playtest unified judgment takeover`，该 commit 已推 main；新对话继续前应先确认 branch 为 `main`、working tree clean、HEAD 与 `origin/main` 一致。
...
当前下一步路线：不要继续分批新建 report，也不要直接推进 generated / golden / final production takeover。v0.0.8.21 已把 legacy final score / accident / type route 隔离成 debug 对照；v0.0.8.22 已补最小 non-final scoring registry / validator gate；v0.0.8.24 开始 score-only playtest takeover scaffold；v0.0.8.25 开始 playtest unified judgment takeover scaffold；v0.0.8.26 已为 drink type composer 准备 stable ingredient composition tags。下一刀若做 composable drink type labeler，应读取 `data/ingredientCompositionTags.js`，不要读 displayName / 中文 category / legacy type。后续调优应继续走 pressure / balance / aggregation / judgment 的通用规则，不得写 recipe whitelist、sample-specific if 或 displayName / 中文名特判。

...
- 最新 candidate：`v0.0.5.31-candidate`
- 最新 candidate commit：`349ba15a88506708ee40a4b652009fc67ff8201b`
- 最新 main：本轮 `v0.0.5.32` commit 提交后以 `git log -1` 为准；本轮不创建 tag，不创建 `v0.0.5.32-candidate`。
- `v0.0.5.31-candidate` 已冻结并推送，指向 `349ba15a88506708ee40a4b652009fc67ff8201b`；正式 tag `v0.0.5.31` 未创建。
...
- v0.0.5.32 已完成 `drinkTypeRules` refs 小批迁移，未创建 `v0.0.5.32` tag。
...
【不要删】v0.0.5.x 后续不默认推进完整三层 summary。v0.0.5.30 已完成 `proportionSegmentRules` refs 小批迁移，`v0.0.5.30-candidate` 已冻结并推送；v0.0.5.31 已完成 `combinationRules` refs 小批迁移，`v0.0.5.31-candidate` 已冻结并推送；v0.0.5.32 已完成 `drinkTypeRules` refs 小批迁移，当前未创建 `v0.0.5.32-candidate`。下一步可考虑 feedbackTag / 文案池边界复查，或保存 result / 历史快照边界复查；后续还可考虑 ID 化收口审计。允许少量 legacy 逻辑暂存，但不能继续扩张；新增结构应优先 stable ID + displayName / text，但不要为未来尚不存在的系统提前造空架子。

...
v0.0.7.81 曾作为 docs-only mechanism closure / handoff 记录“can enter closure mode”，但当前 P0 docs recovery 已经优先级更高。

在 P0-A / P0-B 修复并经 ChatGPT + 用户确认前，不能把 v0.0.7.x 视为安全 closure，也不能开 v0.0.8.x。
...
下一步应先完成 P0 docs recovery review，再由 ChatGPT + 用户决定是否恢复 closure、补正式版本记录、或进入后续 planning stage。

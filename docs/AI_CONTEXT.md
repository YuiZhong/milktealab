...
- 最新 candidate：`v0.0.6.9-candidate`
- 最新 candidate commit：`86f3c357c234accd5a5b3c1545eb327692b4e74d`
- `v0.0.6.9-candidate` 已冻结并推送，指向 `86f3c357c234accd5a5b3c1545eb327692b4e74d`；正式 tag `v0.0.6.9` 未创建。
- 最新 main：本轮 docs commit 是 `v0.0.6.9-candidate` 之后的状态同步 commit，提交后以 `git log -1` 为准。
...
- golden samples 当前应为 `20/20 passed`。
- v0.0.6.10 已完成三层 summary 中段复盘，本地 commit 后以 `git log -1` 为准。
...
- 当前仍未实现 summary -> candidate runtime，未新增 candidate engine，未创建 `v0.0.6.10-candidate`。
- 下一步可考虑 `v0.0.6.10-candidate` 冻结，或 `v0.0.6.11｜summary -> candidate docs / schema 复核`，或 `v0.0.6.11｜summary candidate 只读地基`。不要把下一步写成已经决定，不要为了“干净”批量迁移全部规则表，也不要为未来尚不存在系统提前造空架子。

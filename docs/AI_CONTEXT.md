...
- 最新 main：本轮 docs commit 是 `v0.0.6.11-candidate` 之后的状态同步 commit，提交后以 `git log -1` 为准。
...
- 当前仍未实现 summary -> candidate runtime，未新增 candidate engine，当前未推进 v0.0.6.12。
- 下一步可考虑 `v0.0.6.12｜summary candidate 只读地基`，或 `v0.0.6.12｜candidate golden 结构断言设计`。不要把下一步写成已经决定，不要为了“干净”批量迁移全部规则表，也不要为未来尚不存在系统提前造空架子。

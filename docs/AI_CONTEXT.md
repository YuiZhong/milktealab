...
当前 verified baseline：latest main 为 `b5b3df7 chore: add ingredient profile value draft workspace`，该 commit 已推 main；新对话继续前应先确认 branch 为 `main`、working tree clean、HEAD 与 `origin/main` 一致。
...
- 不开放 active generated severity / final takeover；不直接改 final score / feedback / result.type / golden expected。
...
下一步路线：不要继续分批新建 report，也不要开始 v0.0.8.12 以外的路线。下一步应是 `v0.0.8.12 all-37 source-referenced profile value first pass`，直接基于 `content_sheets/drafts/ingredient_profile_value_draft.v0.0.8.11.csv` / `.json` 生成下一版草案工作台，例如 v0.0.8.12 CSV / JSON。

v0.0.8.12 必须覆盖 37/37 原料；不允许只做 13 个后继续向后推进。每个原料至少要有 `sourceNotes`、`confidence`、`reviewStatus`、`aiComment`。资料不足的原料也必须标明 low confidence / source_limited / needs follow-up，不能空着跳过。

v0.0.8.12 仍不新增 report，不新建 planning 文档，不改 runtime profile，不写 `data/generated`，不改 scoring logic，不改 final result，不为了目标分数或校正测试反推 ingredient profile。联网资料是参考基线，不是游戏真理；最终仍需制作人 review。

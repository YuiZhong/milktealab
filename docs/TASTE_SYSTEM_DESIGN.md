...
- flavor identity conflict outcome 方向：`outcomeTypeId=taste_conflict`，当前不绑定 `bubble_conflict`，因为该 tag 语义偏气泡 + 厚重 / 口感冲突追评，不代表当前 outcome 接管。
...
- `outcomeTypeId`：结果 / outcome 机制大类，例如 `taste_conflict`。它不等于 `sourceLayer=taste`；名字里有 taste 也不代表一定来自 `tasteSummary`。如果它来自 flavor identity conflict 后进入 taste_conflict outcome，应通过 `sourceLayer`、`triggerMetric` 和 notes 解释来源。

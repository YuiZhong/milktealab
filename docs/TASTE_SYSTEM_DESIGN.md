...
这个旧线性优先级只保留为 historical rough rule：它表达“严重翻车不能被好组合洗白”的早期粗原则，但不再作为当前完整判定模型。

当前判定层级、priority vs severity 边界和所有判定层反 if 地狱原则，以 `docs/TASTE_DECISION_MODEL.md` 为准。
...
严重翻车必须优先被看见。好组合只能加分，不能覆盖事故。但“优先被看见”不等于扣分更多；最终 severity、`scoreMultiplier`、反馈强度和阈值调优应由后续 severity / evidence / triggerMetric / thresholds / weights 决定。
...
底层 `tasteProfile` / `textureProfile` / `flavorProfile`、summary 和 future `triggerMetric` 应优先支持 numeric values / numeric load。高 / 中 / 低、light / medium / heavy 或中文档位只作为 human-readable label / review hint / display helper；正式计算、severity 判断、threshold 和 `scoreMultiplier` 不应只依赖文字档位。人类编辑入口可以同时提供数值 + 中文解释，但机器计算应以数值字段为准。
...
本轮只沉淀 numeric-first 设计原则，不填写任何正式数值，不创建 profile 表，也不创建 severity 表。

### 1.2 跨层顾客偏好 / audience tolerance 原则

未来顾客偏好 / audience tolerance 不应只接 taste 层。它应预留跨层接入能力，能够读取或调节：

- taste 层：甜、酸、苦、咸、涩、刺激等基础味觉体验。
- texture / drinkability 层：八宝粥感、水泥感、奶脂油腻、胶粘感等口感和可饮用性体验。
- flavor / structure 层：强风味压制、风味身份错位、主题过载、清爽被厚重吞掉等风味和饮品结构体验。
- severity / feedback / score 层：同一机制对不同客群可以有不同容忍度、扣分、反馈强度或正负向解释。

同一高强度体验对不同客群可能是扣分、可接受，甚至加分。例如：小孩可能喜欢高甜或小料很多的八宝粥感；上班族 / 咖啡爱好者可能接受更高苦味；茶爱好者可能接受更强涩感；猎奇客群可能接受更强刺激或强风味。

当前不实现顾客系统，不创建 `customerTag` / `audienceId`，不改评分，也不新增正式 schema。未来应通过 preference profile、audience tags、tolerance、rule table、metadata 等数据化方式表达，而不是写成 `if 小孩 then 喜欢甜`、`if 上班族 then 喜欢苦咖啡` 这类硬编码。

### 1.3 Texture / mouthfeel 四类成熟主边界

Texture / mouthfeel 当前成熟的主边界至少包括四类：小料固体负载、粉泥低流动性、奶脂油腻负担、糖浆胶质黏稠挂口。它们都可能被玩家说成“厚 / 重 / 喝着累”，但机制不同，不能合并成泛泛 `texture_overload`。

- `texture_solid_overload` 关注 `solidLoad` / `toppingLoad` / `chewLoad` / `liquidSupport`；核心体验是小料多、要嚼、像八宝粥 / 甜品碗，但不一定吸不上来。
- `texture_low_drinkability` 关注 `slurryLoad` / `pasteLoad` / `powderLoad` / `lowFlowPenalty`；它才是粉泥、糊状、水泥感、流动性差和吸管吃力的主边界。
- 奶脂油腻负担应作为 future texture / mouthfeel deferred concept；它不是吸不动，而是饮用流畅但油腻、奶脂压口、糊嘴、下咽后恶心或几口后腻住。
- 糖浆胶质黏稠挂口应作为 future texture / mouthfeel deferred concept；它是黏、挂口、糖浆感、胶质感，不等于奶脂油腻。

`mouthCoating` 目前可以作为相邻方向，但未来需要继续审它是共用指标还是应拆成更具体指标。
...
### 1.5 Ingredient profile factual baseline and scoring calibration｜原料事实基线与评分校准

Ingredient profile 是材料事实层 / 游戏化相对属性层，不是为了让测试分数好看的调参垃圾桶。

未来批量原料数值草案应尽量先理解材料本身。参考来源可以包括 pH、糖度、苦味 / 涩感、脂肪含量、固体负载、质地、黏度、风味身份、食品常识、论文和食品成分资料。科学数据是参考基线，不是游戏真理；最终仍要转成适合游戏手感的相对数值。

游戏化校正也必须尊重材料本质：柠檬应是高酸，厚乳 / 淡奶油应有奶脂负担，珍珠 / 奥利奥 / 芋泥应贡献不同类型 texture load。不得为了让某杯饮料得到 95 分，就把柠檬酸度、厚乳 `fatLoad`、奥利奥 solid / sediment load 等改到不真实。

不要为了只是让一杯饮料通过校正测试而修改原材料数值，让它在错误的、不精确的系统里得出一个符合人类直觉的分数。

根据玩家 / 制作人反馈调数值时，优先调评分 / 判定系统，而不是调原料事实。如果一杯人类觉得应该高分但系统低分，应优先检查 threshold 是否过严、`severityLevel` / `scoreMultiplier` 是否太狠、positive synergy 是否缺失、drinkType expectation 是否没参与、客群偏好是否缺失、score aggregation 是否不合理，以及 accident 与 good combo balance 是否失衡。只有当原料 profile 本身确实不符合现实 / 相对尺度错误时，才调整原料 profile。
...
- `outcomeTypeId`：可选兜底结果过滤，例如 `flavor_identity_conflict`、`experimental_special`。为空表示不限兜底结果。legacy `taste_conflict` 只表示 v0.0.7.41 前的迁移前历史 ID。
...
- 风味身份 / 香气压力反馈方向：当前不填 `feedbackTag`，只在 `notes` 中说明 `aroma_pressure` 仍需 registry / schema 确认后才能成为 runtime 文案池 tag。
- flavor identity conflict outcome 方向：`outcomeTypeId=flavor_identity_conflict`，v0.0.7.41 已从 legacy `taste_conflict` 迁移；当前不绑定 `bubble_conflict`，因为该 tag 语义偏气泡 + 厚重 / 口感冲突追评，不代表当前 outcome 接管。
...
AI / Codex 批量生成的 ID 不能因为“看起来像 stable ID”就默认正确。英文 ID 不等于好设计。ID 一旦进入 docs、sample sheet、generated data、golden 或 runtime，就可能被未来 AI 当成事实来源。因此所有 AI 生成 / 辅助生成的机制相关 ID，都必须先经过层级确认和语义审计。

ID 层级分类：

- `ingredientId`：原料身份，例如 `fruit_lemon`、`dairy_milk`、`topping_oreo_crumble`。它可以包含原料类别前缀，因为它本来就是原料；它不等于事故机制，也不等于 sampleId。
- `accidentTypeId`：事故机制大类，例如 `taste_acid_overload`、`texture_straw_resistance`、`dairy_fat_overload`。它不能按每个原料拆分，不能带 severity 后缀，不能带 sampleId 语义。错误示例包括 `taste_acid_overload_lemon`、`taste_acid_overload_hawthorn`、`taste_acid_overload_high`、`extreme_lemon_accident`；正确示例是 `accidentTypeId: taste_acid_overload`。
- `outcomeTypeId`：结果 / outcome 机制大类，例如 `flavor_identity_conflict`。它不等于 runtime `feedbackTag`，也不能只靠 ID 名字反推 `sourceLayer`。legacy `taste_conflict` 是 v0.0.7.41 前迁移前 ID；如果 outcome 来自 flavor identity conflict，应通过 `sourceLayer=flavor`、`sourceSummary=flavorSummary`、`triggerMetric=identityConflictRisk` 和 notes 解释来源。
- `drinkTypeId`：饮品类型身份，例如 `classic_milk_tea`。它不等于玩家自定义饮品名，也不等于具体 `recipeId` / `recipeVersionId`。
- `feedbackTag`：文案选择标签。只有已经在 feedback 文案池、registry 或 generated data 中登记并确认过的 tag，才应作为稳定 `feedbackTag` 使用。`aroma_pressure` 这类 summary / candidate 风险名不能自动写进 `feedbackTag`；`bubble_conflict` 这类已有旧 tag 也必须确认语义适合当前机制场景，不能因为“稳定存在”就塞进新机制示例。
- `textId`：单条反馈文案 ID，只标识具体文案，不代表机制，不能当事故类型、`ruleId` 或 `candidateId` 使用。
- `sampleId`：开发测试样本身份，只用于 golden samples / review pack / 测试定位。它不进入正式机制规则主键，不等于 `accidentTypeId`，也不等于玩家可见事故名。`extreme_lemon_accident` 只是某个测试样本，不代表“柠檬事故机制”。
- `ruleId`：规则行身份，可以描述规则用途和 severity 档位，例如 `taste_acid_overload_high`；它不是 `accidentTypeId`。`ruleId` 可以带 draft / high / medium / low 等规则语义，`accidentTypeId` 不可以。
- `candidateId`：candidate 层候选身份，不等于最终事故、outcome 或 drinkType。candidate 可以引用 `accidentTypeId` / `outcomeTypeId` / `drinkTypeId`，但自身不是最终结果。
- `priorityBand`：粗优先级分组，不等于 severity，也不等于最终扣分。
- `severityHint`：candidate 层提示，不是最终 severity，不是最终扣分档，也不应直接推出 `scoreMultiplier`。
- `severityLevel`：未来真正严重度层级，只在 severity / threshold 调参层使用。不应混进 `accidentTypeId`，也不应偷藏在 summary / candidate builder 中影响 runtime。
- `sourceLayer` / `sourceSummary` / `triggerMetric`：来源和触发指标，应引用结构化 summary / candidate 字段，不应使用中文显示名、文案片段或 sampleId。不能仅根据 ID 字符串前缀反推 `sourceLayer`；历史 ID 可能包含原料或内容语义，例如 `dairy_fat_overload`，但如果规则行写明 `sourceLayer=texture`、`sourceSummary=textureSummary`、`triggerMetric=fatLoad`，就应按 texture / drinkability 方向理解，ID 名称不能覆盖结构字段。
- draft / sample 后缀：draft 可以出现在样例表里的 `ruleId`，表示这条规则只是草案；draft 不应出现在正式 `accidentTypeId` / `outcomeTypeId` / `drinkTypeId`。sample 语义只属于 sampleId / 测试样本，不应进入机制规则主键。
- `displayName` / `zhCN` / `notes`：人类可读展示和备注，永远不能作为机制主键。notes 可以解释草案状态，但不能替代结构字段去歧义。

草案 ID / sample sheet guardrail：

- sample CSV / JSON 虽然不接 runtime，但会影响未来 AI 理解，因此不能随便填稳定字段。
- 草案行应保持 `enabled=FALSE`，草案 `ruleId` 应明确 draft 状态。
- 未注册的 tag 不应写入稳定字段；如果某字段只是未来可能值，应写入 `notes`，而不是填进 stable ID 字段。
- 如果样例表使用旧系统已有 tag，也必须确认语义适合当前机制场景。
- 样例表不能为了展示方便，把 sampleId、displayName 或 `zhCN` 放进机制主键字段。

AI / Codex 新增机制相关 ID 前必须自问：

1. 这个 ID 属于哪一层：ingredient / accident / outcome / drinkType / feedback / text / sample / rule / candidate / severity？
2. 它是不是把原料、severity、sample、displayName 混进了不该混的层？
3. 它是否已有现有 stable ID 可以复用？
4. 它是正式 ID，还是 draft / sample / notes？
5. 它是否会被未来 validator / generated data / golden / review pack 引用？
6. 如果这个 ID 被新对话的 AI 看到，会不会误解它是机制事实？
7. 是否在用 ID 字符串前缀猜测来源层，而不是读取 `sourceLayer` / `sourceSummary` / `triggerMetric`？
8. 是否需要用户 / ChatGPT 做机制命名 review？

错误示例与正确示例：

```text
错误：accidentTypeId: taste_acid_overload_lemon
错误：accidentTypeId: taste_acid_overload_hawthorn
正确：accidentTypeId: taste_acid_overload
补充：lemon / hawthorn / passionfruit 只能作为 recipe evidence；sampleId 只用于具体 golden / review test sample。

错误：accidentTypeId: taste_acid_overload_high
正确：ruleId: taste_acid_overload_high
正确：accidentTypeId: taste_acid_overload
正确：severityLevel: high

错误：ruleId: extreme_lemon_accident
错误：accidentTypeId: extreme_lemon_accident
正确：sampleId: extreme_lemon_accident
正确：accidentTypeId: taste_acid_overload

错误：feedbackTag: aroma_pressure
原因：如果 aroma_pressure 只是 flavor risk / candidate tag，尚未注册为 runtime 文案池 feedbackTag。
正确：feedbackTag 为空；notes 说明 aroma_pressure 是未来 feedbackTag candidate，必须注册后才能使用。

错误：feedbackTag: bubble_conflict
原因：如果该行表达 generic flavor identity conflict，而 bubble_conflict 语义偏气泡 + 厚重 / 口感冲突追评。
正确：feedbackTag 为空；notes 说明 feedbackTag 必须随 shadow data / registry 另行确认。

错误：仅因为 accidentTypeId 以 dairy 开头，就推断 sourceLayer=flavor 或 dairy 专属 flavor identity conflict。
正确：读取 sourceLayer=texture、sourceSummary=textureSummary、triggerMetric=fatLoad 后，按 texture / drinkability 方向理解 dairy_fat_overload 这条历史机制 ID。
```

validator / registry guardrail：

- future validator 的合法性应以 known stable ID registry / enum / schema 为准。
- 这不是一句口号；validator 实现前必须先明确这些 known stable ID 从哪里来。
- 如果当时没有可用的 registry / enum / schema，就不能直接实现“猜测式 validator”，而应先做一刀 docs / schema 任务，定义 ID source of truth。
- 可选来源包括现有 data / rules 中已登记的 stable ID、统一 ID registry、generated schema，或明确维护的 enum / allowed values。
- 不能只靠字符串后缀或 substring 判断合法性；字符串 lint 可以作为 warning / hint，但不是最终合法性来源。
- 错误方向：
...
如果某个合法 ID 看起来像带原料或 severity 词，也应以 registry / enum / schema 为准，不能被字符串规则误杀。

后续 validate candidate severity sheet 开工前，必须先检查是否已有 known stable ID source；若没有，应先做 registry / enum / schema 设计。本轮仍不新增 registry 文件、不实现 validator、不做全量 ID 审计，只把 validator 的前置条件写入 guardrail。
...
审计对象包括 `accidentTypeId`、`outcomeTypeId`、`drinkTypeId`、`feedbackTag`、`textId`、`sampleId`、`ruleId`、`candidateId`、`priorityBand`、`severityHint`、`severityLevel`、`sourceLayer`、`sourceSummary`、`triggerMetric`，以及 profile / tag / generated sample 中的 draft ID。审计目标是检查是否按原料拆机制、是否把 sampleId 混进机制主键、是否把 severity 后缀混进 `accidentTypeId`、是否把显示文案 / `zhCN` / displayName 当主键、是否把草案 tag 当正式 tag、是否误用旧 tag 语义，以及 `ruleId` / `candidateId` / `accidentTypeId` 是否层级混用。本轮只写 guardrail，不做全量审计、不重命名现有 ID、不新增 registry / validator。

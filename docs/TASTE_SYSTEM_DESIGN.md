...
当前阶段边界已进一步收口：v0.0.5.x 优先保证现有对象身份稳定，解决玩家可见中文不再长期承担系统主键的问题；ID 化完成前，不急着启动完整三层 summary。事故类型、饮品类型、受众类型、反馈标签、规则引用和 golden samples 断言都应逐步走 stable ID + `displayName` / `text` 双轨。三层 profile / summary 正式开工放到 v0.0.6.x；数值调优和 severity 放到 v0.0.7.x 更合适。
...
同一原则也适用于味觉系统输出的事故类型、饮品类型、反馈标签、客群标签等玩家可见文本。中文名称是显示文案，不是系统身份；后续事故、饮品类型、客群和图鉴相关结构应逐步采用 `typeId` / `displayName` 双轨。golden samples 后续也应支持 ID 断言，而不是只依赖中文 `typeIncludes`。

...
- `texture_taro_overload`：未来倾向迁向 `texture_low_drinkability`；芋泥个性保留在 evidence / notes / feedback copy。
- `texture_oreo_overload`：未来倾向迁向 `texture_low_drinkability`；奥利奥粉渣 / 沉积 / 吸管困难保留在 evidence / notes / feedback copy。
- `texture_topping_overload`：未来倾向迁向 `texture_solid_overload`；具体小料名保留在 evidence / notes / feedback copy。
...
| taro paste overload branch | `texture_taro_overload` | `core/accidentAnalyzer.js` | taro paste ratio above 50 | yes, taro paste | no for matching | `texture` | `pasteLoad` / `solidLoad` / `strawResistance` | yes: score cap, type `实验特调`, feedback tags | Migration candidate: ingredient-specific accidentTypeId | keep legacy; future rule should express texture burden without making accidentTypeId per ingredient | before accidentAnalyzer migration route |
| Oreo overload branch | `texture_oreo_overload` | `core/accidentAnalyzer.js` | Oreo crumble ratio above 40 | yes, Oreo crumble | no for matching | `texture` | `sedimentRisk` / `solidLoad` / `strawResistance` | yes: score cap, type `口感事故`, feedback tags | Migration candidate: ingredient-specific accidentTypeId | keep legacy; review whether it maps to texture solid / sediment mechanism | before accidentAnalyzer migration route |
...
- Some IDs are ingredient-specific, such as `flavor_durian_overload`, `texture_taro_overload`, and `texture_oreo_overload`. Future mechanism tables should not multiply accidentTypeId by every ingredient.
...
| Migration candidate, not immediate rename | `flavor_durian_overload`, `texture_taro_overload`, `texture_oreo_overload`, `texture_topping_overload` | Keep now; if future rename is needed, plan compatibility, generated data updates, and golden expected changes deliberately. v0.0.7.45 records future targets for taro / Oreo / topping texture content-specific accidents, but does not migrate them. |

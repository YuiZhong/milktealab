...
>
> 当前 v0.0.7.x docs-only closure checkpoint reached / pending next decision 下，本文件仍是 active stage TODO；但它不是长期正本，阶段结束后必须移出长期必读。
>
...
本文件当前是 v0.0.7.x docs-only closure checkpoint reached / pending next decision 下的 active stage TODO；v0.0.7.x 完结后必须移出长期必读列表。

它不是长期正本。当前处于 v0.0.7.x docs-only closure checkpoint reached / pending next decision 时，它可以作为阶段内必读；v0.0.7.x 完结后应移出长期必读列表。
...
P0 docs recovery / pause mode 已结束。当前项目处于 v0.0.7.x docs-only closure checkpoint reached / pending next decision。

P0-A / P0-B / P0-C 已解决，但这不代表 v0.0.7.x 已完成 formal safe closure / candidate / formal tag，也不代表 P1 solved。v0.0.8.x 尚未开放。
...
## Docs-only closure checkpoint reached / pending next decision

当前 docs-only closure checkpoint reached / pending next decision 目标：
...
5. 准备 docs-only closure checkpoint / candidate discussion，或在 ChatGPT + 用户确认 closure path 后创建下一阶段 active TODO。

`reports/v0.0.7ClosureAuditP1Split.md` 记录 P1-1..P1-8 的 closure split 审计结果；该 report 不表示 P1 solved，也不开放 v0.0.8.x / generated severity / generated severity shadow / partial / active takeover。Existing generated feedback shadow, if present, remains non-final and does not affect final feedback / final result.

`reports/v0.0.7SafeClosureDecision.md` 记录 P0-A / P0-B / P0-C 已 resolved，P1 split 已足以进入 safe closure discussion / closure checkpoint preparation；该 report 不表示 P1 solved，也不开放 v0.0.8.x / generated severity / generated severity shadow / partial / active takeover。剩余债务必须迁移到下一阶段 active stage TODO 后，才能进入任何 next-stage implementation。

`reports/v0.0.7ClosureCheckpointDiscussion.md` 记录 closure checkpoint / candidate discussion readiness；该 report 不创建 candidate / formal tag，不开放 v0.0.8.x，不表示 P1 solved。Remaining debts must migrate to next-stage active TODO before implementation.

`reports/v0.0.7ClosureCheckpoint.md` records the docs-only closure checkpoint；该 report 不创建 candidate / formal tag，不开放 v0.0.8.x，不表示 P1 solved。Remaining debts must migrate to next-stage active TODO before implementation.

当前限制：

- v0.0.7.x 尚未完成 formal safe closure / candidate / formal tag。
- v0.0.8.x 尚未开放。
- P1 不等于 solved。
...
- P0：P0-A / P0-B / P0-C 已解决，项目从 P0 docs recovery / pause mode 转入 v0.0.7.x docs-only closure checkpoint reached / pending next decision。
- P1：存在若干机制 / ID / validator / legacy 迁移债务；这些债务不能伪装成 solved。v0.0.7.x safe closure 前必须明确分流：收口前给结论、迁移到下一阶段 active TODO，或保留为 staged legacy support。迁移后的债务不得在未建 next-stage active TODO 前进入 implementation。
...
### v0.0.7.72 texture drinkability boundary line

v0.0.7.72 开始 texture drinkability boundary review，新增 `reports/textureDrinkabilityBoundaryReview.v0.0.7.72.md`。

本轮记录三者人话边界：

- `texture_solid_overload`：八宝粥感 / 小料太多；根因是固体 / 小料负载太高，液体支撑不足。
- `texture_low_drinkability`：水泥感 / 粉浆感 / 喝不动；根因是整杯可饮用性崩坏，偏糊化、沉积、低流动性、粉浆感。
- `texture_straw_resistance`：吸管很累；暂作吸管层面的表现 / 指标 / 相邻机制，不自动等于前两者。

当前边界：

- `texture_solid_overload` 第一批线已阶段性收口，本轮不改 scaffold、不改 status。
- `texture_low_drinkability` 仍 pending，本轮不加入 scaffold。
- `texture_straw_resistance` final decision 仍 pending，本轮不加入 scaffold。

本轮不改 registry scaffold / check script，不改 runtime / generated / content sheets / golden，不新增 proposal / sample pack / 大 review pack，也不表示 P1 solved。

### v0.0.7.73 texture_low_drinkability next-batch candidate draft

v0.0.7.73 新增 `reports/textureLowDrinkabilityCandidateDecisionDraft.v0.0.7.73.md`，只判断 `texture_low_drinkability` 是否可以进入下一批 accidentTypeId candidate decision draft。

当前建议：

- `texture_low_drinkability` 人话为水泥感 / 粉浆感 / 喝不动。
- recommendedDecision 为 `candidate_for_next_batch_after_boundary_notes`。
- 它和 `texture_solid_overload` / `texture_straw_resistance` 的边界沿用 v0.0.7.72 结论。
- `texture_taro_overload` / `texture_oreo_overload` 只作为 historical migration evidence，不回流 current registry。

当前边界：

- 本轮不把 `texture_low_drinkability` 加入 scaffold。
- 本轮不批准 `texture_low_drinkability`。
- `texture_straw_resistance` final decision 仍 pending。
...
### v0.0.7.74 texture_low_drinkability source notes

v0.0.7.74 新增 `reports/textureLowDrinkabilitySourceNotes.v0.0.7.74.md`，只为 `texture_low_drinkability` 补 source notes / triggerMetric notes / evidence boundary。

当前 source notes：

- `texture_low_drinkability` 人话为水泥感 / 粉浆感 / 喝不动。
- sourceLayer draft 为 `texture`。
- sourceSummary draft 为 `textureSummary / structure texture summary`。
- triggerMetric candidates 为 `drinkabilityPenalty` / `flowBreakdown` / `pasteLoad` / `sedimentLoad` / `powderSlurryLoad`。
...
### v0.0.7.75 texture_low_drinkability candidate notes record

v0.0.7.75 新增 `reports/textureLowDrinkabilityCandidateNotesRecord.v0.0.7.75.md`，只把 `texture_low_drinkability` 整理成下一批 accidentTypeId candidate notes record。

当前 candidate notes record：

- `texture_low_drinkability` 人话为水泥感 / 粉浆感 / 喝不动。
- `candidateRecordStatus` 为 `candidate_notes_recorded_not_approved`。
- sourceLayer 为 `texture`。
- sourceSummary 为 `textureSummary / structure texture summary`。
- triggerMetric candidates 仍为 `drinkabilityPenalty` / `flowBreakdown` / `pasteLoad` / `sedimentLoad` / `powderSlurryLoad`，final metric name 仍需后续 review。
...
v0.0.7.78 记录 v0.0.7.x 后半段 scope pivot：不继续在 v0.0.7.x 批量生产 accidentTypeId / feedbackTag / triggerMetric / ingredient profile / severity / threshold 内容。

新的收口方向：

- v0.0.7.x closure 应聚焦 pre-shadow pipeline checklist。
- 已有 `data/stableIdRegistry.js` / `scripts/content/checkStableIdRegistry.js` 保留为 scaffold / read-only check 样板。
- 批量内容生产 deferred 到后续 batch-content stage，likely v0.0.8.x or later，但不是已确定发布计划。
- ordinary IDs 不再一个个走完整长流程。
- Codex 不得自行生成机制概念；概念设计、分类、合并拆分和制作人判断应先由 ChatGPT + 用户完成。
- 下一步建议：`v0.0.7.x closure checklist / pre-shadow pipeline checklist`。
...
### v0.0.7.79 closure checklist / pre-shadow pipeline checklist

v0.0.7.79 新增 `reports/v0.0.7ClosurePreShadowChecklist.v0.0.7.79.md`，把 v0.0.7.x 后半段切入 closure mode。

当前收口方向：

- v0.0.7.x 应聚焦 scaffold / read-only check / guardrail / pre-shadow boundary audit。
- v0.0.7.x 不继续批量内容生产。
- batch accidentTypeId / feedbackTag / triggerMetric / ingredient profile / threshold / severity / score / review pack 延后到后续 batch-content stage，likely v0.0.8.x or later，但不是已确定发布计划。
- Codex 不得自行生成机制概念；机制概念、合并拆分和制作人判断应先由 ChatGPT + 用户完成。
- 下一步建议：`v0.0.7.80｜v0.0.7.x docs / guardrail consistency audit`。

pre-shadow 前必须确认：

- source-of-truth boundary 清楚。
...
- 不接 runtime，不接 generated severity，不做 shadow / partial / active takeover。
- 不从 Codex-only concepts 生成 batch content。
...
### v0.0.7.81 mechanism closure / handoff

v0.0.7.81 新增 `reports/v0.0.7MechanismClosureHandoff.v0.0.7.81.md`，只做 v0.0.7.x mechanism closure / handoff。
...
后续不要继续 expanding v0.0.7.x content。fresh conversation 应先读 `AI_CONTEXT` / `VERSION_LOG` / `V0_0_7_MECHANISM_TODO` / `STABLE_ID_NAMING_GUARDRAIL` / `V0_0_7_ID_SOURCE_OF_TRUTH_DESIGN` / handoff report，再决定是否 tag formal v0.0.7.x closure 或打开后续 planning branch / stage。

本轮不改 registry scaffold / check script，不改 runtime / generated / content sheets / golden，不生成任何新 ID / tag / profile / feedbackTag / triggerMetric，不批准任何 ID，也不表示 P1 solved。
...
- 普通 ID 默认批量走。
- 同层同类 ID 一起 review / source notes / scaffold。
- 高风险特殊 ID 才单独审。
- 不再为普通 ID 重复 decision draft / source notes / candidate notes / readiness / preparation 全套流程。
- Codex 不得自行把批量 ID 直接塞进 runtime / generated severity。
- 批量 ID 进入 scaffold 前仍需 ChatGPT / 用户整体审查与 check script gate。
- 示例 ID 只是未来批量处理示例，不是本轮批准的 ID、registry entry、scaffold entry 或 allowed values。

示例批量单位：

- taste overload / taste imbalance 批：`taste_acid_overload`、`taste_sweet_overload`、`taste_bitter_overload`、`taste_salty_overload`、`taste_astringent_overload`、`taste_flatness` / `taste_weakness`。
- texture / drinkability 批：`texture_low_drinkability`、`texture_straw_resistance`、`texture_solid_overload`、`texture_slurry_overload`、`texture_sediment_pressure`。
- flavor identity / aroma pressure 批：`flavor_identity_conflict`、`aroma_pressure`、`savory_identity_pressure`、`novelty_pressure`、`strong_flavor_dominance`。
- feedbackTag 批：`greasy_overload`、`straw_disaster`、`straw_followup`、`bubble_conflict`、`low_beverage_fit`。
...
- 可能诱导“一个原料一个事故 ID”。
- 可能把主观产品评价变机制 ID。
- 可能影响存档 / runtime / generated severity。
- 可能造成 displayName 主键回潮。

例如 `flavor_durian_overload` / `industrial_creamer_overload` 这类 ID 可以单独审。

本轮不新增 report，不创建任何新 ID，不改 registry scaffold / check script，不改 runtime / generated / content sheets / golden，也不表示 P1 solved。

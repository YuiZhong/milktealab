...
v0.0.6.x 解决“这些东西如何被三层系统判断”。
v0.0.7.x 解决“判断得好不好、数值顺不顺”。
```

### 8.1 v0.0.5.x：现有核心系统 ID 化 / 去中文主键 / 平台无关数据地基阶段

v0.0.5.x 的完成标准不是“三层判定全部正式开工”，而是：

> 现有味觉实验室核心链路中，玩家可见中文基本不再承担系统主键职责。
...
### 8.2 v0.0.6.x：三层 profile / summary / 判定地基阶段

v0.0.6.x 才正式进入三层 profile / summary / 判定系统。
...
三层判定的数据化原则不变：代码负责“怎么判”，数据负责“判什么”，测试负责“别判歪”。summary 是判定地基，不是把旧中文 name if 换一个文件名继续堆。

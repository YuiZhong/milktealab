...
本文档不是完整版本流水账。版本变更记录仍以 `docs/VERSION_LOG.md` 为准；新对话恢复的短摘要仍以 `docs/AI_CONTEXT.md` 为准；味觉系统设计细节仍以 `docs/TASTE_SYSTEM_DESIGN.md` 为准。当前完整判定层级、priority vs severity 边界和所有判定层反 if 地狱原则，以 `docs/TASTE_DECISION_MODEL.md` 为准。本文档负责承载后续底层架构任务的详细笔记，不表示新判定模型已经 runtime 接管。
...
### Profile values vs scoring system boundary

Profile / summary 层描述材料和饮品状态；severity / scoring 层解释这些状态对好喝程度、反馈强度和分数的影响。不要用最终目标分数反向污染 profile。

当 generated severity / score suggestion overlay 与人类直觉冲突时，应先定位问题属于 profile factual issue、threshold issue、severity issue、score aggregation issue、positive synergy issue、drinkType expectation issue 还是 preference issue。Codex 不得为了让 golden pass、校正测试通过或页面分数好看，直接改 profile 数值来迎合目标输出。

任何大规模 profile value changes 都应有资料来源、参考说明、review 和 shadow comparison。若原料 profile 合理但评分不合理，优先修评分系统，而不是把真实材料数值扭歪。
...
当前完整判定层级以 `docs/TASTE_DECISION_MODEL.md` 为准。本文件只记录架构承载方式：candidate / priority shell 可以表达“先看什么”，不能直接推出最终扣分。最终 severity / `scoreMultiplier` 必须由后续调参层按 `docs/TASTE_DECISION_MODEL.md` 设计。
...
所有判定层都不能新增 if 地狱；代码调度，数据判定。legacy if 可以短期保留为 compatibility / migration target，但不能作为新增机制的方式。
...
这类事故不是“好不好喝”，而是服务参数或物理条件直接破坏饮品成立方式。它们应长期进入 service-condition / physical-reaction rules，而不是继续堆具体组合 if。
...
这类事故来自 `textureProfile` / `textureSummary` / texture rule table / threshold table。它不等于“一有珍珠 / 芋圆 / 芋泥就扣大分”，必须结合比例、液体支撑、饮用场景和 severity 判断。
...
好组合只能在前面没有重大事故时加分，不能洗白特殊服务事故、质地事故、味觉事故或风味冲突。
...
后续其中一部分可能上移到 service / texture / taste / flavor 层；剩余内容不应形成独立于 `docs/TASTE_DECISION_MODEL.md` 的新优先级层。

...
const inputJson = "content_sheets/drafts/ingredient_profile_value_draft.v0.0.8.12.json";
const defaultCsv = "content_sheets/drafts/ingredient_profile_value_draft.v0.0.8.14.csv";
const defaultJson = "content_sheets/drafts/ingredient_profile_value_draft.v0.0.8.14.json";
...
const tastePatches = {
  tea_black: { aromaImpact: 45, bitterness: 42, freshness: 20, heaviness: 10, tea: 65 },
  tea_green: { acidity: 5, aromaImpact: 38, bitterness: 32, freshness: 65, tea: 60 },
  tea_oolong: { aromaImpact: 55, bitterness: 36, freshness: 32, heaviness: 18, tea: 62 },
  tea_jasmine: { aromaImpact: 65, bitterness: 24, freshness: 60, tea: 50 },
  tea_puer: { aromaImpact: 68, bitterness: 52, freshness: -6, heaviness: 42, isStrongAroma: true, tea: 70, viscosity: 10, weirdness: 18 },
  ...
  liquid_coffee: { acidity: 22, aromaImpact: 78, bitterness: 68, heaviness: 28, isStrongAroma: true, weirdness: 18 },
  fruit_lemon: { acidity: 80, aromaImpact: 55, freshness: 72, heaviness: -12, sweetness: 5, viscosity: -6 },
  ...
  flavor_matcha: { aromaImpact: 70, bitterness: 60, freshness: 25, heaviness: 25, isStrongAroma: true, tea: 58, viscosity: 18, weirdness: 18 },
  flavor_cocoa: { aromaImpact: 68, bitterness: 55, freshness: -12, heaviness: 36, isStrongAroma: true, viscosity: 22, weirdness: 12 },
  sweetener_white_sugar: { sweetness: 85 },
...
const texturePatches = {
  ...
  dairy_cream: { drinkabilityPenalty: 18, fatLoad: 82 },
  ...
  topping_pearl: { chewiness: 75, drinkabilityPenalty: 28, liquidSupportNeed: 35, solidLoad: 70, strawResistance: 65 },
  ...
  topping_taro_ball: { chewiness: 70, drinkabilityPenalty: 30, liquidSupportNeed: 38, pasteRisk: 20, solidLoad: 66, strawResistance: 60 },
  ...
  topping_taro_paste: { drinkabilityPenalty: 78, liquidSupportNeed: 85, pasteRisk: 85, sediment: 28, solidLoad: 70, strawResistance: 78 },
...
const flavorPatches = {
  ...
  fruit_durian: { aromaPressure: 92, identityStrength: 95, dominantPotential: 90 },
...
function writeJson(outPath, rows, sourceReferences) {
  const payload = {
    metadata: {
      schemaVersion: "ingredientProfileValueDraft.v0.0.8.14",
      scaleRecalibration: true,
      runtimeData: false,
      generatedData: false,
      draftWorkspaceOnly: true,
      sourceReferencedValuesProvided: true,
      sourceReferenceMode: "scale_recalibrated_from_v0_0_8_12_source_referenced_first_pass",
      canApplyToRuntime: false,
      canAffectFinalScore: false,
      canAffectGoldenExpected: false,
      basedOn: "ingredient_profile_value_draft.v0.0.8.12",
      rowCount: rows.length,
      sourceReferences: sourceReferences || [],
      scaleNotes,
      notes: "All rows are v0.0.8.14 scale recalibration drafts for human review. Proposed values are not runtime data, generated data, official profile values, scoring rules, threshold values, or golden expected changes."
    },
    rows
  };
  fs.mkdirSync(path.dirname(outPath), { recursive: true });
...
function buildRows() {
  ...
  return {
    ...
    rows: payload.rows.map(row => {
      ...
      const scaleNote = "v0.0.8.14 scale recalibration: proposed values now use a clearer 0-100 review scale; current* remains runtime snapshot.";
      ...
      return {
        ...
        proposalMode: "scale_recalibrated_v0_0_8_14_review_only",
        sourceNeeded: "TRUE",
        sourceNotes: appendNote(row.sourceNotes, `${scaleNote}${schemaGapNote}`),
        confidence: row.confidence || "medium",
        reviewStatus: row.reviewStatus || "needs_producer_review",
        producerComment: "",
        aiComment: appendNote(row.aiComment, `按制作人确认的新标尺重校准 proposed 值；仍需制作人 review，不接 runtime。${schemaGapNote}`),
        canApplyToRuntime: "FALSE",
        canAffectFinalScore: "FALSE",
        canAffectGoldenExpected: "FALSE",
        sourceStatus: row.sourceStatus || "source_referenced_first_pass",
        draftStatus: "draft_scale_recalibration_review_only_not_runtime"
      };
    })
  };
}
...
function main() {
  ...
  console.log("Scale recalibration: true");
